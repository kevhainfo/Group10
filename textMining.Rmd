...
df2$required_experience<-gsub(" ", "_", df2$employment_type)
df2$required_experience<-gsub("-", "_", df2$employment_type)
...
fit.min.glm <- glm(fraudulent ~ dLength+pLength+rLength+required_education+LSTM_PREDICT, data2.train, family=binomial)
Anova(fit.min.glm)
fit.min.glm2 <- glm(fraudulent ~ dLength+pLength+rLength+required_education, data2.train, family=binomial)
Anova(fit.min.glm2)
